...
class SolidCacheMongoid::ExpiryTest < ActiveSupport::TestCase
  ...
  [ :thread, :job ].each do |expiry_method|
    test "expires old records (#{expiry_method})" do
      SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0)
    ...
    test "expires records when the cache is full via max_size (#{expiry_method})" do
      ...
      assert_operator SolidCacheMongoid::Entry.count, :<, 4
    end

    test "expires records no shards (#{expiry_method})" do
      SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0)

      @cache = ActiveSupport::Cache.lookup_store(:solid_cache_mongoid_store, expiry_batch_size: 3, namespace: @namespace, max_entries: 2, expiry_method: expiry_method)
    ...
    test "expires when random number is below threshold (#{expiry_method})" do
      SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0.416)

      @cache = ActiveSupport::Cache.lookup_store(:solid_cache_mongoid_store, expiry_batch_size: 3, namespace: @namespace, max_entries: 1, expiry_method: expiry_method)
    ...
    test "doesn't expire when random number is above threshold (#{expiry_method})" do
      SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0.417)

      @cache = ActiveSupport::Cache.lookup_store(:solid_cache_mongoid_store, expiry_batch_size: 6, namespace: @namespace, max_entries: 1, expiry_method: expiry_method)
  ...
  test "triggers multiple expiry tasks when there are many writes" do
    ...
    SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0.25, 0.24)
  ...
  test "triggers multiple expiry jobs when there are many writes" do
    ...
    assert_enqueued_jobs(2, only: SolidCacheMongoid::ExpiryJob) do
      @cache.write_multi(8.times.index_by { |i| "key#{i}" })
    end

    assert_enqueued_jobs(5, only: SolidCacheMongoid::ExpiryJob) do
      @cache.write_multi(24.times.index_by { |i| "key#{i}" })
    end

    # Whether we overflow an extra job depends on rand
    SolidCacheMongoid::Store.any_instance.stubs(:rand).returns(0.125, 0.124)
    assert_enqueued_jobs(2, only: SolidCacheMongoid::ExpiryJob) do
      @cache.write_multi(10.times.index_by { |i| "key#{i}" })
    end
